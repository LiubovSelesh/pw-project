...
test.describe('Form Layouts page', () => {
    ...
    test('radio buttons', async({page}) => {
        ...
        expect(await usingTheGridForm.getByRole('radio', {name: "Option 1"}).isChecked()).toBeFalsy  // uselected
...
test('tooltips', async({page}) => {
    ...
    const tooltipTipCard = page.locator('nb-card', {hasText: "Tooltip Placements"})
    await tooltipTipCard.getByRole('button', {name: "Top"}).hover()
...
test('dialog boxes', async({page}) => {   //this test  show us how to automated dialog boxes which belongs to browse
...
test('datepicker desired date', async ({page}) => {
    await page.getByText('Forms').click()
    await page.getByText('Datepicker').click()
